...
def group_pixels(ctr, offsets, polar=False):
    ...
    if polar:
        distance[:,:,1] = torch.add(torch.fmod(torch.add(distance[:,:,1],width/2),width),-width/2)
...
def get_panoptic_segmentation(instmap, sem, ctr_hmp, offsets, thing_list, label_divisor=2**16, void_label=0,
    ...
    # 这里的inst中心直接由一次kernel为5的maxpool得到，而聚类是直接用offset后，所有点（包括空的）的最近中心获得
    ...
    instmap = torch.argmax(instmap, dim=0, keepdim=True)
    instance[instmap==0]=0

    # 这里也没有对上面得到的中心进行二次聚类，完全依赖于heatmap的maxpool结果
