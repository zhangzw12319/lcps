...
class BEV_Unet(nn.Module):
    ...
    def __init__(self,input_dim,dilation = 1,group_conv=False,input_batch_norm = True,dropout = 0.5,circular_padding = True, dropblock = True):
        super(BEV_Unet, self).__init__()
        self.network = UNet(input_dim, dilation, group_conv, input_batch_norm, dropout,
                            circular_padding, dropblock)

    def forward(self, x):
        center = self.network(x)

        return center
    
class UNet(nn.Module):
    def __init__(self, input_dim,dilation,group_conv,input_batch_norm, dropout,circular_padding,dropblock):
        ...
        self.i_up4_center = up(256, 32, circular_padding, group_conv = group_conv, use_dropblock=dropblock, drop_p=dropout)
        # instance head
